...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Update observer to target relevant sections
    document.querySelectorAll('.search-section, .results-section, .product-card').forEach((el) => {
        observer.observe(el);
    });

    // Button animations
    const buttons = document.querySelectorAll('.btn, .add-to-cart, .delete-btn');
...
function updateResultsCount() {
    resultsCount.textContent = `${filteredProducts.length} book${filteredProducts.length !== 1 ? 's' : ''} available`;
}
...
function renderProducts() {
    ...
    if (filteredProducts.length === 0) {
        productsContainer.innerHTML = '<p class="text-center text-muted">No products found. Please search for a book.</p>';
        updateResultsCount();
    ...
    updateResultsCount();
    clearResultsBtn.style.display = filteredProducts.length > 0 ? 'inline-block' : 'none';
}
...
searchForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const query = searchForm.querySelector('input[name="search"]').value.trim().toLowerCase();
    filteredProducts = products.filter((product) =>
        product.title.toLowerCase().includes(query) || product.author.toLowerCase().includes(query)
    );
    currentPage = 1;
    renderProducts();
    setupPagination();
...
// Event delegation for quantity controls, add to cart, and delete
document.addEventListener('click', function (e) {
    ...
    if (e.target.classList.contains('delete-btn') || e.target.closest('.delete-btn')) {
        ...
        card.addEventListener(
            'animationend',
            () => {
                filteredProducts = filteredProducts.filter((product) => product.title !== title);
...
// Input validation for quantity fields
